...
class VirtualStickFragment : DJIFragment() {
    ...
    private fun initBtnClickListener() {
        ...
        binding?.btnStartMotors?.setOnClickListener {
            virtualStickVM.startMotorsByCSC(object : CommonCallbacks.CompletionCallback {
                override fun onSuccess() {
                    ToastUtils.showToast("CSC Motor Start initiated.")
                }

                override fun onFailure(error: IDJIError) {
                    ToastUtils.showToast("CSC Motor Start failed: $error")
                }
            })
        }
        binding?.btnStopMotors?.setOnClickListener {
            basicAircraftControlVM.turnOffMotors(object :
                CommonCallbacks.CompletionCallbackWithParam<EmptyMsg> {
                override fun onSuccess(t: EmptyMsg?) {
                    ToastUtils.showToast("turnOffMotors onSuccess.")
                }

                override fun onFailure(error: IDJIError) {
                    ToastUtils.showToast("turnOffMotors onFailure,$error")
                }
            })
        }
    }
